...
const isBasicAction = (
  action: TBaseAction | TActionWithAck
): action is TBaseAction => {
  return "input" in action && !("response" in action);
};

const isIoActionWithAck = (
  action: TBaseAction | TActionWithAck
): action is TActionWithAck => {
  return "input" in action && "response" in action;
};
...
type RecursivelyMergeContracts<Contracts extends IoContract[]> =
  Contracts extends [infer Head, ...infer Tail]
...
export {
  getResponseUnionSchema,
  isIoAction,
  isBasicAction,
  isIoActionWithAck,
  isIoListener,
};
export type { RecursivelyMergeContracts };
